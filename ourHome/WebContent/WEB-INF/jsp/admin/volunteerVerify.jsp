...
<script>
  layui.use(['table','form','laydate'], function(){
	  ...
	  table.render({
	    elem: '#test'
	    ,url:'${pageContext.request.contextPath}/user/selectVerify_v'
	    ...
	    ,cols: [[
	      ...
	      ,{field: 'uname', title: '申请人', width:115}
	      ,{field: 'telephone_u', title: '联系电话', width: 157}
	      ,{field: 'root', title: '身份', width: 130}
	      ...
	      ,{field: 'examine_v', title: '审核状态', width: 120,templet:'#setExamine'}
	    ...
	    switch(obj.event){
	      ...
	      case 'LAYTABLE_SEARCH': //table重载
	    	  ...
	    	  table.reload('test', { //表格的id
                  url:'${pageContext.request.contextPath}/user/selectVerifyByName_v?uname='+uName, 
              });
	      break;
	  ...
	  table.on('tool(test)', function(obj){ 
	    ...
	    if(obj.event === 'del'){
	      layer.confirm('确定删除吗', function(index){
	        obj.del();
	        window.location.href="${pageContext.request.contextPath}/verify/delVerify_v?uid="+id;
	        layer.close(index);
	      });
	    } else if(obj.event === 'verify'){
	    //将对应的信息渲染到审核表单
	    	if(data.volunteers.examine_v == 0 || data.volunteers.examine_v == 2){
	    	form.val('popupForm', {
	    		   "uid": data['uid']
	    	      ,"uname": data['uname']
	    	      ,"usex": data['usex']
	    		  ...
	    		  ,"occupation": data.volunteers.occupation
	    		  ,"specialty": data.volunteers.specialty 
	    		  ,"intention": data.volunteers.intention
	    		  ,"experience": data.volunteers.experience
	    		  ,"other": data.volunteers.other
	    	      ,"close": false //开关状态
	    	    });
	    	}else{
	    		form.val('popupForm', {
	  ...
	  form.on('switch(switchListener)', function(data){
		...
		form.on('submit(demo2)', function (data) {
			...
			var examine_v = document.getElementById("examine_v").value;
			if(examine_v == 1){
				$.ajax({
					url   :"${pageContext.request.contextPath }/verify/updateVerifyInfo_v",
					type  :"post",
				...
				$.ajax({
					url   :"${pageContext.request.contextPath }/verify/updateVerifyInfo_v2",
					type  :"post",
					data  :JSON.stringify(data.field),
					contentType :"application/json",
					dataType :"json",
...
<script type="text/html" id="setExamine">
			  {{#  if(d.volunteers.examine_v === 0){d.volunteers.examine_v = '未审核' }}
                 <span lay-event="verify" class="layui-btn layui-bg-orange layui-btn-xs">{{d.volunteers.examine_v}}</span>
              {{#  } else if(d.volunteers.examine_v === 2) {d.volunteers.examine_v = '审核未通过'}}
               <span lay-event="verify" class="layui-btn layui-btn-danger layui-btn-xs">{{d.volunteers.examine_v}}</span>
			  {{#  } else {d.volunteers.examine_v = '已审核'}}
               <span lay-event="verify" class="layui-btn layui-bg-green layui-btn-xs">{{d.volunteers.examine_v}}</span>
              {{#  } }}
</script>
...
<div class="layui-form-item">
  ...
  <input name="root" class="layui-input" type="hidden" value="2" autocomplete="off">
  <input name="examine_v" id="examine_v" class="layui-input" type="hidden" value="2" autocomplete="off">
